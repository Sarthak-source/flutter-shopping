...
class _AddButtonDoubleState extends State<AddButtonDouble> {
  ...
  Widget build(BuildContext context) {
    ...
    focusNode = FocusNode();
    itemScrollController = ItemScrollController();
    int minOrder = double.parse(widget.minOrder).toInt();
    int dblQty = double.parse(widget.qty.toString()).toInt();
    int indexOfQuantity =
        generateList(minOrder, listLength).indexOf(dblQty);
    ...
    if (indexOfQuantity == -1) {
      ...
      //log("test ${generateList(int.parse(widget.minOrder), listLength).indexOf(quantity).toString()}");
    }
                                          ...
                                          onTap: () {
                                            setState(() {
                                              quantity = generateList(
                                                  int.parse(widget.minOrder),
                                                  listLength)[i];
                                            });
                                            widget.onChangedPressed(
                                                "${generateList(int.parse(widget.minOrder), listLength)[i]}");
